...
namespace BT
{
SuspendNode::SuspendNode(const std::string& name, unsigned milliseconds)
  : DecoratorNode(name, {})
...
SuspendNode::SuspendNode(const std::string& name, const NodeConfiguration& config)
...
NodeStatus SuspendNode::tick()
{
    if (read_parameter_from_ports_)
    {
        if (!getInput("delay_msec", msec_))
        {
            throw RuntimeError("Missing parameter [delay_msec] in SuspendNode");
        }
    }
