 ...
 * </Delay>
 */
class SuspendNode : public DecoratorNode
{
  public:
    SuspendNode(const std::string& name, unsigned milliseconds);

    SuspendNode(const std::string& name, const NodeConfiguration& config);

    ~SuspendNode() override
    {
        halt();
    }
